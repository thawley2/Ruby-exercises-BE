...
RSpec.describe Werewolf do
  ...
  it 'consumes a victim' do
    ...
    expect(werewolf.victims).to eq([victim1])
  end

  it 'cannot consume a victim if it is in human form' do
    # your code here
    victim1 = Victim.new
    werewolf = Werewolf.new('Thomas', 'Littleton')
    expect(werewolf.eat(victim1)).to eq('You cannot eat people while in human form.')
  end
